...
const addListeners = function(){
  ...
  const updateBoundsTuples = () => this.boundsTuples = cy.nodes(canBeInBoundsTuple).map((n) => getBoundsTuple(n, options.includeLabels));
  ...
  this.addListener('grab', 'node', e => {
    ...
    if( canPullFromParent(node) ){
      ...
      this.dropTargetBounds = getBoundsCopy(this.dropTarget, options.includeLabels);
    }
  ...
  this.addListener('add', 'node', e => {
    ...
    if( canBeInBoundsTuple(newNode) ){
      this.boundsTuples.push( getBoundsTuple(newNode, options.includeLabels) );
    }
  });
  ...
  this.addListener('drag', 'node', () => {
    ...
    if( this.dropTarget.nonempty() ){ // already in a parent
      const bb = expandBounds( getBounds(this.grabbedNode, options.includeLabels), options.outThreshold );
      ...
      const bb = expandBounds( getBounds(this.grabbedNode, options.includeLabels), options.overThreshold );
      ...
      if( overlappingNodes.length > 0 ){ // potential parent
        ...
        this.dropTargetBounds = getBoundsCopy(parent, options.includeLabels);
